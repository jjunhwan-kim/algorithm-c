...
int pushHeap(Heap* heap, MyData* data)
{
    ...
    // compare function returns 1 if the first parameter has a higher priority than the second parameter, and 0 otherwise.
    while (currentIndex > 0 && heap->compare(heap->data[currentIndex], heap->data[parentIndex]))
    {
        int parentIndex = (currentIndex - 1) / 2;

        MyData* temp = heap->data[parentIndex];
        heap->data[parentIndex] = heap->data[currentIndex];
...
int popHeap(Heap* heap, MyData** data)
{
    ...
    *data = heap->data[0];
    ...
    while (currentIndex * 2 + 1 <= heap->size - 1)
    {
        ...
        {
            if (heap->compare(heap->data[currentIndex * 2 + 1], heap->data[currentIndex * 2 + 2]))
            {
                childIndex = currentIndex * 2 + 1;
            }
            else
